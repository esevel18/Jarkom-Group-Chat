...
public class ChatClient {
	...
	private void processServerResponse(String response) {
		...
		switch (action) {
			case "msg":
				if (parts.length >= 3)
					listener.onMessageReceived(parts[1], parts[2]);
				break;
			case "info":
				if (parts.length >= 2)
					listener.onInfoReceived(parts[1]);
				break;
			case "roomlist":
			...
			case "roomclosed":
				if (parts.length >= 2)
					listener.onKickedOrClosed(parts[1]);
				break;
			case "error":
				if (parts.length >= 2) {

					listener.onInfoReceived("Error: " + parts[1]);
			 ...
			 */
			case "image":
				String senderImage = parts[1];
				String imageFile = parts[2];
				listener.onImageReceived(senderImage, imageFile);
				break;
			case "video":
				String senderVideo = parts[1];
	...
	public void kickUser(String targetUsername) {
		out.println("kick#" + targetUsername);
	}

	/**
	 * Method to send Image or Video
	 * 
	 * @param fileName
	 */
