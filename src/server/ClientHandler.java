...
public class ClientHandler implements Runnable {
	...
	private void processMessage(String message) {
		...
		switch (action) {
			...
			case "kick" -> {
				if (currRoom != null && ChatServer.activeRooms.containsKey(currRoom.getRoomName())
						&& currRoom.getOwner().equals(this.userName)) {
					...
					if (target != null && currRoom.getMembers().contains(target)) {
						...
						target.leaveCurrentRoom(); // leaveCurrentRoom akan memanggil broadcastMemberList secara
													// otomatis
					}
				} else {
			...
			/**
			 * Handler for Image or Video
			 */
			case "image" -> {
				if (currRoom != null)
					currRoom.broadcastImage("image#" + this.userName + "#" + text, this);
			}
			case "video" -> {
				if (currRoom != null)
